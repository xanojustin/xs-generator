function "check_verification" {
  description = "Check a verification code via Twilio Verify API"
  input {
    text to_phone filters=trim { description = "Phone number being verified (E.164 format, e.g., +1234567890)" }
    text code filters=trim { description = "The verification code entered by the user" }
  ...
  stack {
    ...
    var $api_url { value = "https://verify.twilio.com/v2/Services/" ~ $input.service_sid ~ "/VerificationCheck" }
    ...
    conditional {
      if ($twilio_response.response.status >= 200 && $twilio_response.response.status < 300) {
        ...
        var $valid_value { value = $response_body|get:"valid" }
        conditional {
          if ($valid_value == true) {
